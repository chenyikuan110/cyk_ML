...
class hFFLSTMRNN:
    ...
    def __init__ (self, lenIn, lenOut, lenRec, sizeHidden, lenInRec, hiddenRec,\
                  ...
                  learningRate, dropout_threshold, double_fc):
        ...
        self.W2  = np.zeros((sizeHidden,sizeHidden)) # for the second last fully connected layer
        self.GW2 = np.zeros((sizeHidden,sizeHidden)) # Gradient, for W-update using RMSprop
        self.b2  = np.zeros(sizeHidden)
        self.Gb2 = np.zeros(sizeHidden)
        ...
        self.h2s = np.zeros((lenRec,sizeHidden))
    ...
    def fwd_pass(self): 
        ...
        for t in range(0,self.lenRec):
            ...
            if(self.double_fc == 1):
                self.h2s[t] = ReLu(np.dot(self.W2,self.hs[t]) + self.b2)
    ...
    def bwd_pass(self):        
        ...
        if(self.double_fc == 1):
            W2_grad  = np.zeros((self.sizeHidden,self.sizeHidden))
            b2_grad  = np.zeros(self.sizeHidden)
...
#                    print("ERR!",self.ys[t][self.targets[t]]," ",np.sum(self.ys[t]))
    ...
    def inference(self,x):
        ...
        if(self.double_fc == 1):
            self.h2 = ReLu(np.dot(self.W2,self.h) + self.b2)
